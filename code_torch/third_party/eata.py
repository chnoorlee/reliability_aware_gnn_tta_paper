# Vendored from the official EATA implementation:
#   https://github.com/mr-eggplant/EATA  (file: eata.py, main)
#   Efficient Test-Time Model Adaptation without Forgetting, Niu et al., ICML 2022.
#
# License: MIT — Copyright (c) 2023 Shuaicheng Niu, Jiaxiang Wu, Yifan Zhang,
#   Yaofo Chen, Shijian Zheng, Peilin Zhao, Mingkui Tan.  Based on Tent (ICLR'21).
# The full MIT license text is reproduced in code_torch/third_party/ATTRIBUTION.md.
#
# This file is the authors' original source.  Two minimal, clearly marked changes
# adapt it from image CNNs to graph neural networks WITHOUT altering the EATA
# algorithm (entropy-filtered + redundancy-filtered BN adaptation with a diagonal
# Fisher anti-forgetting regularizer):
#   [graph-port 1] the BatchNorm type check is generalized from BatchNorm2d to
#                  include BatchNorm1d (graph features are 1D: [num_nodes, C]).
#   [graph-port 2] the single-argument forward ``model(x)`` is satisfied by a
#                  GraphModelWrapper (see ../baselines_official.py) that stores
#                  edge_index and forwards ``gnn(x, edge_index)``; here "samples"
#                  are graph nodes, so the per-sample entropy/redundancy filters
#                  operate at node granularity.
# The filtering, reweighting, Fisher/EWC loss, and moving-average probability
# logic are byte-for-byte the upstream implementation.
...
# [graph-port 1] BatchNorm types EATA may adapt (upstream: BatchNorm2d only).
...
def collect_params(model):
    ...
    for nm, m in model.named_modules():
        if isinstance(m, _BN_TYPES):  # [graph-port 1]
...
def check_model(model):
    ...
    assert not has_all_params, "eata should not update all params: " \
                               "check which require grad"
    has_bn = any([isinstance(m, _BN_TYPES) for m in model.modules()])  # [graph-port 1]
